...
#Copy files
# Use 'cp /etc/passwd .' to copy the system configuration file to the current directory, using the dot (.) as the relative destination.Z is for Zebra   @darwinalcarraz-hash ➜ /workspaces/UNIX-02-SIN-C-Mar-Jul-2026-Alcarraz2.0 (linux_cisco_1) $ cp /etc/passwd .
@darwinalcarraz-hash ➜ /workspaces/UNIX-02-SIN-C-Mar-Jul-2026-Alcarraz2.0 (linux_cisco_1) $ cp /etc/passwd .
# 'ls' is executed to list the contents and validate that the copy of file 'passwd' was successfully made to the current location.
@darwinalcarraz-hash ➜ /workspaces/UNIX-02-SIN-C-Mar-Jul-2026-Alcarraz2.0 (linux_cisco_1) $ ls 
Calculadora                 cifrado_firmado_final_CayeC.txt.gpg  doc_no_cifrado.txt.sig         gnupg_script.sh
Cayetano_llave_publica.asc  da_llave_publica.asc                 doc_no_cifrado_CayeC.txt       hello.sh
LICENSE                     doc_cifrado.txt                      doc_no_cifrado_CayeC.txt.gpg   linux_cisco_1.sh
README.md                   doc_cifradoCAYE.txt                  doc_no_cifrado_CayeC.txt.sig   llave_privada_clase.asc
alpha.txt                   doc_no_cifrado.txt                   doc_no_cifrado_firmado.txt     passwd
animals.txt                 doc_no_cifrado.txt.gpg               final_cifrado_firmado.txt.gpg  verificacion_Cayetano.txt
#"The copy (cp) operation requires read permissions on the source and write/execute permissions on the destination. This exercise demonstrates the use of absolute paths for the source (/etc/passwd) and the use of directory shortcuts (.) for the destination, optimizing file management within the Linux environment."

## Run 'dd' to create a 50MB file full of binary zeros; 'if' defines the source (/dev/zero), 'of' the destination (/tmp/swapex), 'bs' the block size (1M) and 'count' the number of blocks.
@darwinalcarraz-hash ➜ /workspaces/UNIX-02-SIN-C-Mar-Jul-2026-Alcarraz2.0 (linux_cisco_1) $ dd if=/dev/zero of=/tmp/swapex bs=1M count=50 
50+0 records in
50+0 records out
52428800 bytes (52 MB, 50 MiB) copied, 0.0476341 s, 1.1 GB/s
#"The dd command operates at the bit level, allowing for precise control over data streams. In this case, by interfacing with the virtual device /dev/zero, we generated a 50MB file with a specific block size, demonstrating how Linux manages raw data and low-level file creation for administrative tasks like swap file setup."